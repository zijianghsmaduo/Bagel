...
def measure_editing_with_thinking(prompt, image, enable_taylorseer=False):
  ...
  with profile(
    ...
    profile_memory=True,
    with_stack=True,
    schedule=torch.profiler.schedule(wait=1, warmup=1, active=1, repeat=1), # Optional: for stable measurements
  ...
  # tracefile_name = "outputs/trace/editing_inference_with_thinking_trace.json"
...
def measure_understanding(prompt, image):
  inference_hyper=dict(
    max_think_token_n=1000,
    do_sample=False,
    # text_temperature=0.3,
  ...
  tracefile_name = "outputs/trace/understanding_inference_trace.json"
  prof.export_chrome_trace(tracefile_name)
  print(f"Trace file '{tracefile_name}' has been saved.")
  print("Open it in Chrome by navigating to 'chrome://tracing' or at https://ui.perfetto.dev/")
...
if __name__ == "__main__":
  ...
  # measure_editing_with_thinking("Could you display the sculpture that takes after this design?", image2, enable_taylorseer=True)

  image3 = Image.open('test_images/meme.jpg')
  measure_understanding("Can someone explain what’s funny about this meme??", image3)
