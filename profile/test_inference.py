...
if __name__ == "__main__":
	...
	if args.attn_backend == "naive_cal_ave_self_attn_score" and base_attention is not None:
		ave_self_attn_scores = base_attention.get_ave_self_attn_scores()
		# --- 格式化输出 ---
		print("\n" + "="*60)
		print(" " * 10 + "Average Self-Attention Scores Summary")
		print("="*60)
		
		# 表头
		print(f"{'Layer':<10} | {'Average Self-Attention Score':>30}")
		print("-"*60)

		for cfg_type in ["normal", "cfg_text", "cfg_img"]:
			if cfg_type in ave_self_attn_scores:
				avg_score = ave_self_attn_scores[cfg_type].mean()
				print(f"{cfg_type.replace('_', ' ').title():<10} | {avg_score:>29.6f}")
		print("-"*60)