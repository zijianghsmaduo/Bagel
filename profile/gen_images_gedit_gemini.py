...
from modeling.basic.startup import add_argus_arguments, init_attn_backend_mlp_args
...
The planning process is enclosed within <think> </think> tags, i.e. <think> planning process here </think> image here'''
        ...
        kv_struct.system_prompt = (og_lens[0], newlens[0]-1)
    ...
    kv_struct.vae = (og_lens[0], newlens[0]-1)
    ...
    if use_vit:
        ...
        kv_struct.vit = (og_lens[0], newlens[0]-1)
        with torch.amp.autocast("cuda", enabled=True, dtype=torch.bfloat16):
            generation_input = move_generation_input_to_device(generation_input, device)
    ...
    kv_struct.input_prompt = (og_lens[0], newlens[0]-1)
    ...
    if use_think: 
        ...
        kv_struct.gen_text = (og_lens[0], newlens[0]-1)
        with torch.amp.autocast("cuda", enabled=True, dtype=torch.bfloat16):
            generation_input = move_generation_input_to_device(generation_input, device)
            past_key_values = model.forward_cache_update_text(past_key_values, **generation_input)  
...
def shuffle_list(original_indices, shuffle_base=1):
    """Shuffle a list with a given seed."""
    import random
    # random.seed(seed)
    random.shuffle(original_indices)
    shuffle_size = len(original_indices) // shuffle_base
    shuffle_indices = original_indices[:shuffle_size]
    return shuffle_indices

def save_metrics_to_csv(
    sparsity_data: dict, 
    similarity_data: dict,
    avg_self_attn_data: dict, 
    key: str, 
    sparsity_save_path: str, 
    similarity_save_path: str,
    avg_self_attn_save_path: str
):
    """
    ...
    if avg_self_attn_data and avg_self_attn_save_path:
      avg_self_attn_records = []
      for cfg_type, data_array in avg_self_attn_data.items():
          # data_array shape is (49, 28)
          if data_array.size == 0 or np.all(data_array == 0): continue

          # 直接计算所有 timestep 和 layer 的总平均值
          total_mean = np.mean(data_array)

          record = {
              "key": key,
              "cfg_type": cfg_type,
              "avg_self_attn_score": total_mean
          }
          
          avg_self_attn_records.append(record)

      if avg_self_attn_records:
          df_avg_self_attn = pd.DataFrame(avg_self_attn_records)
          df_avg_self_attn.to_csv(
              avg_self_attn_save_path, 
              mode='a', 
              header=not os.path.exists(avg_self_attn_save_path), 
              index=False
          )
...
def process_dataset(
    ...
    print(f"Total samples before shuffling: {len(idx_list)}")
    idx_list = shuffle_list(idx_list, shuffle_base=shuffle_base)
    print(f"Total samples after shuffling: {len(idx_list)}")
    ...
    for data_idx in tqdm(idx_list):
        ...
        save_path_fullset_avg_self_attn = f"{output_dir}/fullset/{task_type}/{instruction_language}/{key}_avg_self_attn.csv"
        ...
        os.makedirs(os.path.dirname(save_path_fullset_avg_self_attn), exist_ok=True)
        ...
        print(f"Processing the {data_idx}th image {key} with instruction: {instruction}")
        ...
        try:
            ...
            save_metrics_to_csv(
                ...
                avg_self_attn_data=base_attn.average_self_attn_scores if base_attn is not None else {},
                key=key,
                sparsity_save_path=save_path_fullset_sparsity,
                similarity_save_path=save_path_fullset_similarity,
                avg_self_attn_save_path=save_path_fullset_avg_self_attn
            )
            if base_attn is not None:
              base_attn.clear_sparsity()
              base_attn.clear_ave_self_attn_scores()
...
def main():
    ...
    parser.add_argument("--shuffle_base", type=int, default=1, help="Whether to shuffle the base indices")
    ...
    # )
    base_attention, mlp_args = init_attn_backend_mlp_args(args)
