...
def editing_with_text_img_cfg(
    ...
    # ensure we modify the module-level thinking_len variable
    global thinking_len
...
The planning process is enclosed within <think> </think> tags, i.e. <think> planning process here </think> image here'''
    ...
    if use_think: 
        with torch.amp.autocast("cuda", enabled=True, dtype=torch.bfloat16):
            ...
            # print(unpacked_latent.shape)
            thinking_len += unpacked_latent.shape[0]
            output = tokenizer.decode(unpacked_latent[:,0])
            think_output = output.split('<|im_end|>')[0].split('<|im_start|>')[1]
            print(f"Thinking Output: {think_output}")
...
def process_dataset(
    ...
    # print(f"Average thinking length: {thinking_len / len(idx_list)}")
    output_txt_file = f"{output_dir}/thinking_lengths_{shard_id}.txt"
    with open(output_txt_file, "w") as f:
        f.write(f"Total thinking length: {thinking_len}\n")
