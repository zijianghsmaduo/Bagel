...
from modeling.basic.attention import set_new_threshold, get_sparsity
...
if __name__ == "__main__":
	...
	parser.add_argument("--nr_tasks", type=int, required=True, help="Number of tasks to run")
	parser.add_argument("--task_id", type=int, required=True, help="Task ID to run")
	args = parser.parse_args()

	min_threshold_range = [0.0, 4e-5]
	step = 1e-7
	total_rounds = (min_threshold_range[1] - min_threshold_range[0]) / step
	rounds_per_task = total_rounds / args.nr_tasks
	start_round = int(args.task_id * rounds_per_task)
	end_round = int((args.task_id + 1) * rounds_per_task)
	min_threshold_range = [min_threshold_range[0] + start_round * step, min_threshold_range[0] + end_round * step]
	...
	for threshold in np.arange(min_threshold_range[0], min_threshold_range[1], step):
		...
		set_seed(seed)
		editing_inference_with_thinking("Could you display the sculpture that takes after this design?", image2, save_path=f"{save_dir}/editing_inference_with_thinking_output.jpg")
